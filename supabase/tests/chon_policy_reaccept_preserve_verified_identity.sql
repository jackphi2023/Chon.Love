...
insert into auth.users(
  ...
  'authenticated','authenticated','chon-policy-reaccept@example.test','',
...
-- Simulate an account that existed when the Chon.Love grandfather migration ran.
insert into auth.users(
  instance_id,id,aud,role,email,encrypted_password,raw_app_meta_data,raw_user_meta_data,
  created_at,updated_at,confirmation_token,recovery_token,email_change_token_new,
  email_change_token_current,phone_change,phone_change_token,reauthentication_token
...
select is((select age_verified from public.get_my_onboarding_status()), true, 'grandfathered existing user bypasses the age gate');
select is((select policies_accepted from public.get_my_onboarding_status()), true, 'grandfathered existing user bypasses stale policy routing');
...
-- A user created after the migration has no marker and retains the normal gate.
insert into auth.users(
  instance_id,id,aud,role,email,encrypted_password,raw_app_meta_data,raw_user_meta_data,
  created_at,updated_at,confirmation_token,recovery_token,email_change_token_new,
  email_change_token_current,phone_change,phone_change_token,reauthentication_token
...
select is((select age_verified from public.get_my_onboarding_status()), false, 'future under-18 user does not inherit grandfathering');
select is((select policies_accepted from public.get_my_onboarding_status()), true, 'future user may still satisfy current policy acceptance independently');
