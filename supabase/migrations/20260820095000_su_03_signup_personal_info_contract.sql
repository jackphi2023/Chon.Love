-- Chon.Love Signup / Onboarding V2 — SU-03/SU-04
...
-- Relationship status is the single relationship/marital-state field; no duplicate
-- marital_status type or column is introduced.
...
begin
  ...
  -- Stricter Signup V2 validation is isolated to incomplete profiles so historical
  -- active members are not invalidated by newer registration rules.
  ...
  -- DOB stays private and the existing versioned policy/18+ authority remains
  -- the single source of truth. SU-04 removes only the redundant visible checkboxes.
  ...
  -- Username remains a backend/public-profile invariant but is hidden from the
...
revoke all on function public.save_my_signup_personal_info_v2(
  ...
  public.education_level,public.relationship_status,public.children_status,
  public.drinking_status,public.smoking_status
) from public, anon;
...
comment on function public.save_my_signup_personal_info_v2(
  ...
  'SU-03/SU-04 staged personal-info write for incomplete Signup V2 profiles. Validates 18+, display name 10-50 and height 120-220; optional factual fields may be not shared; relationship_status is the only relationship/marital field; DOB/policy acceptance stays private and versioned; username is auto-generated only when missing; profile is not activated.';
