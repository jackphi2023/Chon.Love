-- Session D / D-FIX01
-- Keep the Admin member-photo verification queue operationally safe as it grows:
-- newest cases must be visible on the first page and pagination must remain stable.
...
comment on function public.admin_list_member_photo_verifications(uuid, integer, integer) is
  'Session D Admin photo-verification queue: newest-first with deterministic id tie-break and bounded offset pagination; service-role only.';
