...
-- OPT-12: reopen the authenticated withdrawal surface only after the user UX exists.
-- Keep the server switch as an emergency fail-closed control and preserve all LX-19
-- KYC, verified-bank, financial-hold, minimum-balance, atomic-ledger and idempotency checks.
insert into private.app_config(key,value_json,value_type,description,is_public)
values(
  'withdrawal_requests_enabled',
  'true'::jsonb,
  'boolean',
  'OPT-12: allow eligible authenticated members to submit guarded withdrawal requests.',
  false
)
...
begin
  ...
  perform private.require_boolean_config('withdrawal_requests_enabled');
...
revoke execute on function public.request_withdrawal(uuid,bigint,uuid) from public,anon;
grant execute on function public.request_withdrawal(uuid,bigint,uuid) to authenticated,service_role;

comment on function public.request_withdrawal(uuid,bigint,uuid) is
  'OPT-12: authenticated member withdrawal request; fail-closed config plus KYC, verified bank, financial hold, minimum balance, atomic ledger allocation and idempotency are enforced server-side.';
