-- Qualify withdrawal request/cancel columns that collide with OUT parameter names.
...
begin
  ...
  select w.* into v_existing from private.withdrawals w where w.creator_id=v_user_id and w.idempotency_key=p_idempotency_key;
  if found then
    select cea.* into v_account from private.creator_earning_accounts cea where cea.creator_id=v_user_id;
  ...
  select ba.* into v_bank from private.bank_accounts ba where ba.id=p_bank_account_id and ba.user_id=v_user_id and ba.status='verified' and ba.deleted_at is null for update;
  ...
  select (ac.value_json#>>'{}')::bigint into v_minimum from private.app_config ac where ac.key='minimum_withdrawal_units';
  select (ac.value_json#>>'{}')::bigint into v_rate from private.app_config ac where ac.key='heart_vnd_rate';
  select (ac.value_json#>>'{}')::bigint into v_units_per_heart from private.app_config ac where ac.key='heart_units_per_heart';
  ...
  select cea.* into v_account from private.creator_earning_accounts cea where cea.creator_id=v_user_id for update;
  ...
  for v_position in
    select rp.* from private.creator_reward_positions rp
  ...
  loop
    ...
    update private.creator_reward_positions as rp set
      available_units=rp.available_units-v_take,
      held_units=rp.held_units+v_take,
      status=private.reward_position_status_for(rp.original_units,rp.pending_units,rp.available_units-v_take,rp.held_units+v_take,rp.paid_units,rp.reversed_units)
  ...
  update private.creator_earning_accounts as cea set
    available_units=cea.available_units-p_requested_reward_units,
    held_units=cea.held_units+p_requested_reward_units,
    version=cea.version+1
  where cea.creator_id=v_user_id returning cea.* into v_account;
  ...
  update public.economy_sync as es set creator_account_version=v_account.version,updated_at=now() where es.user_id=v_user_id;
...
begin
  ...
  if exists(select 1 from private.admin_audit_logs a where a.request_id=p_request_id and a.action='withdrawal_cancelled' and a.target_id=p_withdrawal_id) then
    select w.* into v_withdrawal from private.withdrawals w where w.id=p_withdrawal_id and w.creator_id=v_user_id;
    select cea.* into v_account from private.creator_earning_accounts cea where cea.creator_id=v_user_id;
    return query select v_withdrawal.id,v_withdrawal.status::text,v_account.available_units,true;
    return;
  end if;
  select w.* into v_withdrawal from private.withdrawals w where w.id=p_withdrawal_id and w.creator_id=v_user_id for update;
  ...
  for v_alloc in select wa.* from private.withdrawal_reward_allocations wa where wa.withdrawal_id=v_withdrawal.id for update loop
    update private.creator_reward_positions as rp set
      held_units=rp.held_units-v_alloc.allocated_units,
      available_units=rp.available_units+v_alloc.allocated_units,
  ...
  where cea.creator_id=v_user_id returning cea.* into v_account;
  update private.withdrawals as w set status='cancelled' where w.id=v_withdrawal.id returning w.* into v_withdrawal;
  insert into private.creator_reward_ledger(creator_id,entry_type,amount_units,available_at,reference_type,reference_id,idempotency_key,metadata_json)
