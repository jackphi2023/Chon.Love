-- Chon.Love Signup / Onboarding V2 — Step 3 UX follow-up
-- Keep the existing staged personal-info authority and relax only the new-signup
-- display-name minimum from 10 to 6 characters. Existing active profiles and the
-- mature profile editor contract are unchanged.
...
begin
  ...
  if char_length(v_display_name) < 6 or char_length(v_display_name) > 50 then
    raise exception using errcode = '22023', message = 'signup display_name must be between 6 and 50 characters';
  end if;
...
comment on function public.save_my_signup_personal_info_v2(
  ...
  'Signup V2 staged personal-info write for incomplete profiles. Validates 18+, display name 6-50 and height 120-220; optional factual fields may be not shared; DOB/policy acceptance remains private/versioned; username is generated only when missing; profile is not activated.';
