...
begin
  ...
  if p_action='approve' then
    ...
    update private.withdrawals set status='approved',reviewed_at=now(),reviewed_by=p_actor_user_id,approved_at=now(),rejection_reason_code=null where id=v_withdrawal.id returning * into v_withdrawal;
    ...
    update private.withdrawals set status='processing' where id=v_withdrawal.id returning * into v_withdrawal;
    ...
    for v_alloc in select * from private.withdrawal_reward_allocations where withdrawal_id=v_withdrawal.id for update loop
      update private.creator_reward_positions set held_units=held_units-v_alloc.allocated_units,available_units=available_units+v_alloc.allocated_units,
    ...
    update private.withdrawals set status='rejected',reviewed_at=now(),reviewed_by=p_actor_user_id,rejection_reason_code=p_reason_code,approved_at=null where id=v_withdrawal.id returning * into v_withdrawal;
    ...
    for v_alloc in select * from private.withdrawal_reward_allocations where withdrawal_id=v_withdrawal.id for update loop
      update private.creator_reward_positions set held_units=held_units-v_alloc.allocated_units,paid_units=paid_units+v_alloc.allocated_units,
      ...
      where gift_transaction_id=v_alloc.gift_transaction_id;
      update private.withdrawal_reward_allocations set paid_units=allocated_units where withdrawal_id=v_withdrawal.id and gift_transaction_id=v_alloc.gift_transaction_id;
    end loop;
    update private.creator_earning_accounts set held_units=held_units-v_withdrawal.requested_reward_units,paid_units=paid_units+v_withdrawal.requested_reward_units,version=version+1
      where creator_id=v_withdrawal.creator_id returning * into v_account;
    update private.withdrawals set status='paid',paid_at=now(),payment_reference=btrim(p_payment_reference) where id=v_withdrawal.id returning * into v_withdrawal;
    insert into private.creator_reward_ledger(creator_id,entry_type,amount_units,available_at,reference_type,reference_id,idempotency_key,metadata_json)
  ...
  if p_action in ('reject','paid') then update public.economy_sync set creator_account_version=v_account.version,updated_at=now() where user_id=v_withdrawal.creator_id; end if;
