...
comment on function public.complete_my_onboarding(date, text, text, text) is
  'Completes adult onboarding. For an already age-verified adult, subsequent policy re-acceptance preserves verified DOB and age verification state and updates only current policy acceptance fields.';
