...
Deno.serve(async (request: Request) => {
  ...
  if (publicError || !publicProfile?.avatar_available) return response(404, 'profile_avatar_not_available');
  ...
  const { data: blob, error: downloadError } = await server.storage
  ...
  return new Response(bytes, {
    ...
    headers: {
      ...corsHeaders,
      'Content-Type': media.mime_type || blob.type || 'image/jpeg',
