...
for (const [label, netlify] of [['root', rootNetlify], ['apps/mobile mirror', mobileNetlify]]) {
  expect(
    netlify.includes('corepack enable && pnpm --filter @myfan/mobile build:web'),
    `${label} Netlify configuration must build the canonical Mobile workspace.`,
  );
...
console.warn('Chon.Love Netlify validation passed: Expo Web is canonical and non-production deploys are not hard-wired to production data.');
