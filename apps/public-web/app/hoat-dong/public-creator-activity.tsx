...
import { createActivityStorageUrl, getYouTubeThumbnail, listCreatorActivity, type CreatorActivityPost } from '@myfan/supabase';
...
type FeedState = { status: 'idle' | 'loading' | 'ready' | 'error'; posts: PublicPost[] };

export function PublicCreatorActivity() {
  const [username, setUsername] = useState('');
  const [draftUsername, setDraftUsername] = useState('');
  const [state, setState] = useState<FeedState>({ status: 'idle', posts: [] });
      ...
      .then(async (posts) => Promise.all(posts.map(async (post) => {
        const source = post.original_bucket && post.original_path
          ...
          : post.preview_bucket && post.preview_path
            ...
            : null;
        const mediaUrl = source ? await createActivityStorageUrl(client, source.bucket, source.path, 30).catch(() => null) : null;
        const avatarUrl = post.avatar_bucket && post.avatar_path ? await createActivityStorageUrl(client, post.avatar_bucket, post.avatar_path, 30).catch(() => null) : null;
        return { ...post, mediaUrl, avatarUrl };
      })))
      .then((posts) => { if (active) setState({ status: 'ready', posts }); })
      .catch(() => { if (active) setState({ status: 'error', posts: [] }); });
    return () => { active = false; };
  ...
  return (
    <>
      <form className="activityPublicLookup" onSubmit={submitUsername}>
        ...
        <div><input autoCapitalize="none" autoComplete="off" id="activity-username" onChange={(event) => setDraftUsername(event.target.value)} placeholder="@username" value={draftUsername} /><button className="primary" type="submit">Xem Hoạt động</button></div>
      </form>
      {!username ? <div className="activityPublicState"><strong>Nhập username Creator</strong><p>Trang public chỉ tải bài đã duyệt và không cho visitor thực hiện giao dịch.</p></div> : null}
      {state.status === 'loading' && state.posts.length === 0 ? <div aria-busy="true" className="activityPublicList">{Array.from({ length: 3 }, (_, index) => <div className="activityPublicCard activitySkeleton" key={index} />)}</div> : null}
      ...
      {state.status === 'ready' && state.posts.length === 0 ? <div className="activityPublicState"><strong>Chưa có Hoạt động công khai</strong><p>Creator chưa có bài đã được duyệt để hiển thị.</p></div> : null}
...
function PublicActivityCard({ post }: { post: PublicPost }) {
  ...
  return (
    <article className="activityPublicCard">
      ...
      {post.content_type === 'image' ? <div className="activityPublicMedia">{post.mediaUrl ? <img alt={`Ảnh Hoạt động của ${post.display_name}`} src={post.mediaUrl} /> : <div className="activityPublicMediaUnavailable">Preview đang được xử lý</div>}{locked ? <div className="activityPublicLock"><span aria-hidden="true">🔒</span><strong>1 ảnh đang được khóa</strong><p>{post.required_gift_icon_emoji} {post.required_gift_name_vi} · {post.required_gift_hearts} ❤️</p><a className="primary" href="/?intent=login">Đăng nhập để tặng {post.required_gift_name_vi ?? 'quà'}</a></div> : null}</div> : null}
