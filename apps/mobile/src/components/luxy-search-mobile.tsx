...
function toSearchInput(draft: DraftFilters, sort: LuxySearchSort): SearchLuxyProfilesInput {
  return {
    ...
    favoriteScope: draft.favoriteScope,
  };
}
...
function activeFilterCount(input: SearchLuxyProfilesInput): number {
  ...
  if (input.viewState) count += 1;
  if (input.favoriteScope) count += 1;
  return count;
}
...
export function LuxySearchMobile() {
  ...
  const profilesQuery = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      if (!client) throw new Error('supabase_not_configured');
      return searchLuxyProfilesV2(client, { ...applied, limit: LUXY_SEARCH_DEFAULT_PAGE_SIZE, offset: pageParam });
    },
      ...
      <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator>
        <View style={styles.toolbar} testID="luxy-search-mobile-toolbar">
          ...
          <View style={styles.toolbarActions}>
            ...
            >
              ...
              {appliedCount ? <View style={styles.filterCountBadge}><Text style={styles.filterCountText}>{appliedCount}</Text></View> : null}
            </Pressable>
            <Pressable
        ...
        {profilesQuery.isLoading && profiles.length === 0 ? (
          ...
          <State>
            ...
            <Pressable accessibilityRole="button" onPress={() => setFiltersOpen(true)} style={styles.secondaryPill}><Text style={styles.secondaryPillText}>Mở bộ lọc</Text></Pressable>
          </State>
        ) : (
        ...
        {profilesQuery.isFetchingNextPage ? (
          ...
          <Pressable accessibilityRole="button" onPress={() => void profilesQuery.fetchNextPage()} style={styles.loadMoreButton}><Text style={styles.loadMoreText}>Xem thêm thành viên</Text></Pressable>
            ...
            <ScrollView contentContainerStyle={styles.filterContent} showsVerticalScrollIndicator>
              <MobileFilterSection title="Khu vực">
                <Pressable accessibilityRole="button" onPress={() => setProvinceOpen((value) => !value)} style={styles.selectControl}>
                  <Text style={styles.selectText}>{selectedProvince?.name ?? 'Toàn Việt Nam'}</Text><Text style={styles.chevron}>⌄</Text>
                </Pressable>
                {provinceOpen ? (
                  <View style={styles.provincePanel}>
                    <Pressable accessibilityRole="button" onPress={() => { setDraft((current) => ({ ...current, provinceId: null })); setProvinceOpen(false); }} style={styles.provinceOption}>
                    ...
                    <ScrollView nestedScrollEnabled style={styles.provinceList}>
                      {(provincesQuery.data ?? []).map((province) => (
                        <ProvinceRow key={province.id} province={province} selected={draft.provinceId === province.id} onPress={() => { setDraft((current) => ({ ...current, provinceId: province.id })); setProvinceOpen(false); }} />
                      ))}
                    </ScrollView>
                  </View>
                ) : null}
              ...
              <MobileFilterSection title="Khoảng cách">
                <View style={styles.chipWrap}>
                  {DISTANCE_OPTIONS.map((distance) => <FilterChip key={distance} active={draft.maxDistanceKm === distance} label={`${distance} km`} onPress={() => setDraft((current) => ({ ...current, maxDistanceKm: distance }))} />)}
              ...
              <MobileFilterSection title="Lịch sử xem">
                <View style={styles.chipWrap}>
                  <FilterChip active={draft.viewState === null} label="Tất cả" onPress={() => setDraft((current) => ({ ...current, viewState: null }))} />
                  <FilterChip active={draft.viewState === 'unviewed'} label="Chưa xem" onPress={() => setDraft((current) => ({ ...current, viewState: 'unviewed' }))} />
                  <FilterChip active={draft.viewState === 'viewed'} label="Đã xem" onPress={() => setDraft((current) => ({ ...current, viewState: 'viewed' }))} />
                </View>
              </MobileFilterSection>

              <MobileFilterSection title="Yêu thích">
                <View style={styles.chipWrap}>
                  <FilterChip active={draft.favoriteScope === null} label="Tất cả" onPress={() => setDraft((current) => ({ ...current, favoriteScope: null }))} />
                  <FilterChip active={draft.favoriteScope === 'favorites'} label="Tôi yêu thích" onPress={() => setDraft((current) => ({ ...current, favoriteScope: 'favorites' }))} />
                  <FilterChip active={draft.favoriteScope === 'favorited_me'} label="Yêu thích tôi" onPress={() => setDraft((current) => ({ ...current, favoriteScope: 'favorited_me' }))} />
                </View>
              </MobileFilterSection>

              <MobileFilterSection title="Tuổi"><RangeInputs leftLabel="Từ" leftValue={draft.minAge} rightLabel="Đến" rightValue={draft.maxAge} onLeftChange={(value) => setDraft((current) => ({ ...current, minAge: value }))} onRightChange={(value) => setDraft((current) => ({ ...current, maxAge: value }))} /></MobileFilterSection>

              <MobileFilterSection title="Thành viên đang tìm">
                <View style={styles.chipWrap}>{LIFESTYLE_OPTIONS.map((item) => <FilterChip key={item.value} active={draft.lifestyleTags.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, lifestyleTags: toggleValue(current.lifestyleTags, item.value) }))} />)}</View>
              </MobileFilterSection>

              <MobileFilterSection title="Tình trạng quan hệ">{RELATIONSHIP_OPTIONS.map((item) => <CheckRow key={item.value} checked={draft.relationshipStatuses.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, relationshipStatuses: toggleValue(current.relationshipStatuses, item.value) }))} />)}</MobileFilterSection>
              <MobileFilterSection title="Chiều cao"><RangeInputs leftLabel="Từ cm" leftValue={draft.minHeightCm} rightLabel="Đến cm" rightValue={draft.maxHeightCm} onLeftChange={(value) => setDraft((current) => ({ ...current, minHeightCm: value }))} onRightChange={(value) => setDraft((current) => ({ ...current, maxHeightCm: value }))} /></MobileFilterSection>
              <MobileFilterSection title="Cân nặng"><RangeInputs leftLabel="Từ kg" leftValue={draft.minWeightKg} rightLabel="Đến kg" rightValue={draft.maxWeightKg} onLeftChange={(value) => setDraft((current) => ({ ...current, minWeightKg: value }))} onRightChange={(value) => setDraft((current) => ({ ...current, maxWeightKg: value }))} /></MobileFilterSection>
              <MobileFilterSection title="Hút thuốc">{SMOKING_OPTIONS.map((item) => <CheckRow key={item.value} checked={draft.smokingStatuses.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, smokingStatuses: toggleValue(current.smokingStatuses, item.value) }))} />)}</MobileFilterSection>
              <MobileFilterSection title="Uống rượu/bia">{DRINKING_OPTIONS.map((item) => <CheckRow key={item.value} checked={draft.drinkingStatuses.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, drinkingStatuses: toggleValue(current.drinkingStatuses, item.value) }))} />)}</MobileFilterSection>
              <MobileFilterSection title="Học vấn">{EDUCATION_OPTIONS.map((item) => <CheckRow key={item.value} checked={draft.educationLevels.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, educationLevels: toggleValue(current.educationLevels, item.value) }))} />)}</MobileFilterSection>
              <MobileFilterSection title="Con cái">{CHILDREN_OPTIONS.map((item) => <CheckRow key={item.value} checked={draft.childrenStatuses.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, childrenStatuses: toggleValue(current.childrenStatuses, item.value) }))} />)}</MobileFilterSection>

              <MobileFilterSection title="Ngôn ngữ"><TextInput accessibilityLabel="Ngôn ngữ, phân tách bằng dấu phẩy" onChangeText={(value) => setDraft((current) => ({ ...current, languagesText: value }))} placeholder="Tiếng Việt, English" placeholderTextColor={luxyColors.softMuted} style={styles.textControl} value={draft.languagesText} /></MobileFilterSection>
              <MobileFilterSection title="Nghề nghiệp"><TextInput accessibilityLabel="Tìm theo nghề nghiệp" onChangeText={(value) => setDraft((current) => ({ ...current, occupationText: value }))} placeholder="Ví dụ: kiến trúc sư" placeholderTextColor={luxyColors.softMuted} style={styles.textControl} value={draft.occupationText} /></MobileFilterSection>
              <MobileFilterSection title="Tìm trong hồ sơ" last><TextInput accessibilityLabel="Tìm trong nội dung hồ sơ" multiline onChangeText={(value) => setDraft((current) => ({ ...current, profileText: value }))} placeholder="Ví dụ: du lịch, ẩm thực…" placeholderTextColor={luxyColors.softMuted} style={[styles.textControl, styles.multilineControl]} value={draft.profileText} /></MobileFilterSection>
        ...
        <View style={styles.modalBackdrop}>
          ...
          <View style={styles.sortSheet} testID="luxy-search-mobile-sort-sheet">
            ...
            {SORT_OPTIONS.map((option) => (
              <Pressable accessibilityRole="radio" accessibilityState={{ checked: option.value === sort }} key={option.value} onPress={() => changeSort(option.value)} style={styles.sortRow} testID={`luxy-search-mobile-sort-${option.value}`}>
                <Text style={styles.sortRowText}>{option.label}</Text>
                <View style={[styles.radioOuter, option.value === sort && styles.radioOuterActive]}>{option.value === sort ? <View style={styles.radioInner} /> : null}</View>
              </Pressable>
            ))}
          </View>
        </View>
...
function ProvinceRow({ province, selected, onPress }: { province: ProvinceOption; selected: boolean; onPress: () => void }) {
  return <Pressable accessibilityRole="button" onPress={onPress} style={[styles.provinceOption, selected && styles.provinceOptionActive]}><Text style={styles.provinceOptionText}>{province.name}</Text>{selected ? <Text style={styles.selectedMark}>✓</Text> : null}</Pressable>;
}
...
function CheckRow({ checked, label, onPress }: { checked: boolean; label: string; onPress: () => void }) {
  return <Pressable accessibilityRole="checkbox" accessibilityState={{ checked }} onPress={onPress} style={styles.checkRow}><View style={[styles.checkbox, checked && styles.checkboxChecked]}>{checked ? <Text style={styles.checkmark}>✓</Text> : null}</View><Text style={styles.checkLabel}>{label}</Text></Pressable>;
}

function DisabledCheckRow({ label, suffix }: { label: string; suffix: string }) {
  return <View accessibilityState={{ disabled: true }} style={styles.checkRow}><View style={[styles.checkbox, styles.checkboxDisabled]} /><Text style={styles.disabledLabel}>{label}</Text><Text style={styles.comingSoon}>{suffix}</Text></View>;
}

function RangeInputs({ leftLabel, leftValue, rightLabel, rightValue, onLeftChange, onRightChange }: { leftLabel: string; leftValue: string; rightLabel: string; rightValue: string; onLeftChange: (value: string) => void; onRightChange: (value: string) => void }) {
...
const styles = StyleSheet.create({
  ...
  toolbar: { minHeight: 72, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 10, borderBottomWidth: 1, borderBottomColor: luxyColors.border },
  toolbarTitleBlock: { flex: 1, minWidth: 0 },
  title: { color: luxyColors.text, fontFamily: luxyTypography.families.display, fontSize: 23, lineHeight: 28, fontWeight: '400' },
  resultCount: { color: luxyColors.muted, fontSize: 11, marginTop: 2 },
  toolbarActions: { flexDirection: 'row', gap: 5, alignItems: 'center' },
  toolbarButton: { minHeight: 44, minWidth: 74, borderWidth: 1, borderColor: luxyColors.borderStrong, borderRadius: luxyRadii.pill, backgroundColor: luxyColors.surface, paddingHorizontal: 9, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, position: 'relative' },
  ...
  loadMoreButton: { alignSelf: 'center', minHeight: 44, borderRadius: luxyRadii.pill, borderWidth: 1, borderColor: luxyColors.ink, justifyContent: 'center', paddingHorizontal: luxySpacing.xl, marginTop: luxySpacing.xl },
  ...
  filterSheet: { width: '100%', maxWidth: 620, maxHeight: '92%', alignSelf: 'center', backgroundColor: luxyColors.surface, borderTopLeftRadius: 18, borderTopRightRadius: 18, overflow: 'hidden' },
  sheetHandle: { width: 38, height: 4, borderRadius: 2, backgroundColor: luxyColors.borderStrong, alignSelf: 'center', marginTop: 8 },
  sheetHeader: { minHeight: 54, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: luxyColors.border, paddingHorizontal: 8 },
  ...
  filterBody: { gap: 7 },
  selectControl: { minHeight: 44, borderWidth: 1, borderColor: luxyColors.borderStrong, borderRadius: luxyRadii.xs, backgroundColor: luxyColors.surface, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 12 },
  ...
  filterChip: { minHeight: 40, borderWidth: 1, borderColor: luxyColors.borderStrong, borderRadius: luxyRadii.pill, justifyContent: 'center', paddingHorizontal: 11, backgroundColor: luxyColors.surface },
  ...
  rangeInput: { minHeight: 44, borderWidth: 1, borderColor: luxyColors.borderStrong, borderRadius: luxyRadii.xs, backgroundColor: luxyColors.surface, paddingHorizontal: 10, color: luxyColors.text, fontSize: 12 },
  rangeDash: { color: luxyColors.softMuted, paddingBottom: 13 },
  textControl: { minHeight: 44, borderWidth: 1, borderColor: luxyColors.borderStrong, borderRadius: luxyRadii.xs, backgroundColor: luxyColors.surface, paddingHorizontal: 10, paddingVertical: 9, color: luxyColors.text, fontSize: 12 },
