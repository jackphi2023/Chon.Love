...
const RELATIONSHIP_OPTIONS: Array<{ value: RelationshipStatus; label: string }> = [
  { value: 'single', label: 'Độc thân' }, { value: 'divorced', label: 'Đã ly hôn' }, { value: 'widowed', label: 'Goá' }, { value: 'open', label: 'Mối quan hệ mở' }, { value: 'complicated', label: 'Phức tạp' },
];
const SMOKING_OPTIONS: Array<{ value: SmokingStatus; label: string }> = [
  { value: 'never', label: 'Không hút thuốc' }, { value: 'socially', label: 'Hút xã giao' }, { value: 'regularly', label: 'Hút thường xuyên' }, { value: 'trying_to_quit', label: 'Đang cố bỏ' },
];
const DRINKING_OPTIONS: Array<{ value: DrinkingStatus; label: string }> = [
  { value: 'never', label: 'Không uống' }, { value: 'socially', label: 'Uống xã giao' }, { value: 'regularly', label: 'Uống thường xuyên' },
];
const EDUCATION_OPTIONS: Array<{ value: EducationLevel; label: string }> = [
  { value: 'high_school', label: 'THPT' }, { value: 'vocational', label: 'Trung cấp / nghề' }, { value: 'college', label: 'Cao đẳng' }, { value: 'bachelors', label: 'Đại học' }, { value: 'masters', label: 'Thạc sĩ' }, { value: 'doctorate', label: 'Tiến sĩ' }, { value: 'other', label: 'Khác' },
];
const CHILDREN_OPTIONS: Array<{ value: ChildrenStatus; label: string }> = [
  { value: 'no_children', label: 'Chưa có con' }, { value: 'has_children', label: 'Đã có con' },
];
...
const DEFAULT_DRAFT: DraftFilters = {
  provinceId: null, maxDistanceKm: null, minAge: '18', maxAge: '99', minHeightCm: '', maxHeightCm: '', minWeightKg: '', maxWeightKg: '',
  relationshipStatuses: [], childrenStatuses: [], smokingStatuses: [], drinkingStatuses: [], educationLevels: [], lifestyleTags: [],
  languagesText: '', occupationText: '', profileText: '', hasPhoto: false, onlineNow: false, viewState: null, favoriteScope: null,
};
...
function toSearchInput(draft: DraftFilters, sort: LuxySearchSort): SearchLuxyProfilesInput {
  return {
    ...
    minHeightCm: optionalNumber(draft.minHeightCm), maxHeightCm: optionalNumber(draft.maxHeightCm),
    minWeightKg: optionalNumber(draft.minWeightKg), maxWeightKg: optionalNumber(draft.maxWeightKg),
    relationshipStatuses: draft.relationshipStatuses, childrenStatuses: draft.childrenStatuses,
    smokingStatuses: draft.smokingStatuses, drinkingStatuses: draft.drinkingStatuses,
    educationLevels: draft.educationLevels, lifestyleTags: draft.lifestyleTags,
    languages: draft.languagesText.split(',').map((item) => item.trim()).filter((item) => item.length >= 2).slice(0, 8),
    hasPhoto: draft.hasPhoto ? true : null, onlineNow: draft.onlineNow ? true : null,
    occupationText: draft.occupationText.trim() || null, profileText: draft.profileText.trim() || null,
    viewState: draft.viewState, favoriteScope: draft.favoriteScope,
  };
}
...
export function LuxySearchDesktop() {
  ...
  const provincesQuery = useQuery({
    queryKey: ['luxy-search', 'provinces', 'VN'], enabled: Boolean(client), staleTime: 24 * 60 * 60 * 1_000, gcTime: 24 * 60 * 60 * 1_000,
    queryFn: async () => { if (!client) throw new Error('supabase_not_configured'); return listActiveProvinces(client); },
  });
  const contextQuery = useQuery({
    queryKey: ['luxy-search', 'context', auth.userId], enabled: Boolean(client && auth.userId), staleTime: 5 * 60_000,
    queryFn: async () => { if (!client) throw new Error('supabase_not_configured'); return getMyDiscoveryContext(client); },
  });
  const profilesQuery = useInfiniteQuery({
    queryKey: ['luxy-search', 'profiles', auth.userId, applied], enabled: Boolean(client && auth.userId), staleTime: 60_000, gcTime: 5 * 60_000, initialPageParam: 0,
    queryFn: async ({ pageParam }) => { if (!client) throw new Error('supabase_not_configured'); return searchLuxyProfilesV2(client, { ...applied, limit: LUXY_SEARCH_DEFAULT_PAGE_SIZE, offset: pageParam }); },
  ...
  function applyFilters(nextDraft = draft, nextSort = sort) {
    try { setFilterError(null); setApplied(toSearchInput(nextDraft, nextSort)); }
    catch { setFilterError('Một số bộ lọc chưa hợp lệ. Vui lòng kiểm tra lại khoảng giá trị.'); }
  }
  function resetFilters() {
    const next = { ...DEFAULT_DRAFT };
    setDraft(next); setSort('distance'); setProvinceOpen(false); setSortOpen(false); setFilterError(null); setApplied(toSearchInput(next, 'distance'));
  }
  async function updateLocation() {
    if (!client) return;
    setLocationBusy(true); setLocationError(null); setLocationMessage(null);
    try {
  ...
  return (
    <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator testID="luxy-search-desktop">
      <View style={styles.frame}>
        <View style={styles.sidebar} testID="luxy-search-filter-rail">
          ...
          <View style={styles.savedSearchRow}><Text style={styles.savedSearchText}>Tìm kiếm đã lưu</Text><Text style={styles.chevron}>⌄</Text></View>
          <SearchActions apply={() => applyFilters()} reset={resetFilters} />

          <FilterSection title="Khu vực">
            <Pressable accessibilityRole="button" onPress={() => setProvinceOpen((value) => !value)} style={styles.selectControl}><Text style={selectedProvince ? styles.selectValue : styles.selectPlaceholder}>{selectedProvince?.name ?? 'Toàn Việt Nam'}</Text><Text style={styles.chevron}>⌄</Text></Pressable>
            {provinceOpen ? (
              <View style={styles.dropdownPanel}>
                <Pressable accessibilityRole="button" onPress={() => { setDraft((current) => ({ ...current, provinceId: null })); setProvinceOpen(false); }} style={styles.dropdownOption}><Text style={styles.dropdownOptionText}>Toàn Việt Nam</Text></Pressable>
                <ScrollView nestedScrollEnabled style={styles.provinceList}>{(provincesQuery.data ?? []).map((province) => <ProvinceRow key={province.id} province={province} selected={draft.provinceId === province.id} onPress={() => { setDraft((current) => ({ ...current, provinceId: province.id })); setProvinceOpen(false); }} />)}</ScrollView>
              </View>
            ) : null}
            <Pressable accessibilityRole="button" disabled={locationBusy} onPress={updateLocation} style={styles.locationButton}><Text style={styles.locationButtonText}>{locationBusy ? 'Đang cập nhật…' : '⌖  Cập nhật vị trí của tôi'}</Text></Pressable>
            <Text style={styles.helperText}>{contextQuery.data?.has_fresh_location ? 'Vị trí đang hoạt động; hệ thống chỉ trả khoảng cách làm tròn.' : 'Bật vị trí để xếp thành viên gần → xa trên toàn Việt Nam.'}</Text>
          ...
          <FilterSection title="Khoảng cách"><View style={styles.chipRow}>{DISTANCE_OPTIONS.map((distance) => <FilterChip key={distance} active={draft.maxDistanceKm === distance} label={`${distance} km`} onPress={() => setDraft((current) => ({ ...current, maxDistanceKm: distance }))} />)}<FilterChip active={draft.maxDistanceKm === null} label="Tất cả" onPress={() => setDraft((current) => ({ ...current, maxDistanceKm: null }))} /></View></FilterSection>
          ...
          <FilterSection title="Yêu thích"><View style={styles.chipRow}><FilterChip active={draft.favoriteScope === null} label="Tất cả" onPress={() => setDraft((current) => ({ ...current, favoriteScope: null }))} /><FilterChip active={draft.favoriteScope === 'favorites'} label="Tôi yêu thích" onPress={() => setDraft((current) => ({ ...current, favoriteScope: 'favorites' }))} /><FilterChip active={draft.favoriteScope === 'favorited_me'} label="Yêu thích tôi" onPress={() => setDraft((current) => ({ ...current, favoriteScope: 'favorited_me' }))} /></View></FilterSection>
          <FilterSection title="Tuổi"><RangeInputs leftLabel="Từ" leftValue={draft.minAge} rightLabel="Đến" rightValue={draft.maxAge} onLeftChange={(value) => setDraft((current) => ({ ...current, minAge: value }))} onRightChange={(value) => setDraft((current) => ({ ...current, maxAge: value }))} /></FilterSection>
          <FilterSection title="Thành viên đang tìm"><View style={styles.tagWrap}>{LIFESTYLE_OPTIONS.map((item) => <FilterChip key={item.value} active={draft.lifestyleTags.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, lifestyleTags: toggleValue(current.lifestyleTags, item.value) }))} />)}</View></FilterSection>
          <FilterSection title="Cân nặng"><RangeInputs leftLabel="Từ kg" leftValue={draft.minWeightKg} rightLabel="Đến kg" rightValue={draft.maxWeightKg} onLeftChange={(value) => setDraft((current) => ({ ...current, minWeightKg: value }))} onRightChange={(value) => setDraft((current) => ({ ...current, maxWeightKg: value }))} /></FilterSection>
          <FilterSection title="Tình trạng quan hệ">{RELATIONSHIP_OPTIONS.map((item) => <CheckRow key={item.value} checked={draft.relationshipStatuses.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, relationshipStatuses: toggleValue(current.relationshipStatuses, item.value) }))} />)}</FilterSection>
          <FilterSection title="Chiều cao"><RangeInputs leftLabel="Từ cm" leftValue={draft.minHeightCm} rightLabel="Đến cm" rightValue={draft.maxHeightCm} onLeftChange={(value) => setDraft((current) => ({ ...current, minHeightCm: value }))} onRightChange={(value) => setDraft((current) => ({ ...current, maxHeightCm: value }))} /></FilterSection>
          <FilterSection title="Hút thuốc">{SMOKING_OPTIONS.map((item) => <CheckRow key={item.value} checked={draft.smokingStatuses.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, smokingStatuses: toggleValue(current.smokingStatuses, item.value) }))} />)}</FilterSection>
          <FilterSection title="Uống rượu/bia">{DRINKING_OPTIONS.map((item) => <CheckRow key={item.value} checked={draft.drinkingStatuses.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, drinkingStatuses: toggleValue(current.drinkingStatuses, item.value) }))} />)}</FilterSection>
          <FilterSection title="Học vấn">{EDUCATION_OPTIONS.map((item) => <CheckRow key={item.value} checked={draft.educationLevels.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, educationLevels: toggleValue(current.educationLevels, item.value) }))} />)}</FilterSection>
          <FilterSection title="Con cái">{CHILDREN_OPTIONS.map((item) => <CheckRow key={item.value} checked={draft.childrenStatuses.includes(item.value)} label={item.label} onPress={() => setDraft((current) => ({ ...current, childrenStatuses: toggleValue(current.childrenStatuses, item.value) }))} />)}</FilterSection>
          <FilterSection title="Ngôn ngữ"><TextInput accessibilityLabel="Ngôn ngữ, phân tách bằng dấu phẩy" onChangeText={(value) => setDraft((current) => ({ ...current, languagesText: value }))} placeholder="Tiếng Việt, English" placeholderTextColor={luxyColors.softMuted} style={styles.textControl} value={draft.languagesText} /></FilterSection>
          <FilterSection title="Nghề nghiệp"><TextInput accessibilityLabel="Tìm theo nghề nghiệp" onChangeText={(value) => setDraft((current) => ({ ...current, occupationText: value }))} placeholder="Ví dụ: kiến trúc sư" placeholderTextColor={luxyColors.softMuted} style={styles.textControl} value={draft.occupationText} /></FilterSection>
          <FilterSection title="Tìm trong hồ sơ" last><TextInput accessibilityLabel="Tìm trong nội dung hồ sơ" multiline onChangeText={(value) => setDraft((current) => ({ ...current, profileText: value }))} placeholder="Ví dụ: du lịch, ẩm thực…" placeholderTextColor={luxyColors.softMuted} style={[styles.textControl, styles.profileTextControl]} value={draft.profileText} /></FilterSection>
        ...
        <View style={styles.results} testID="luxy-search-results">
          ...
          {profilesQuery.isFetchingNextPage ? <View style={styles.loadMoreState}><ActivityIndicator color={luxyColors.ink} /><Text style={styles.stateText}>Đang tải thêm…</Text></View>
            : profilesQuery.hasNextPage ? <Pressable accessibilityRole="button" onPress={() => void profilesQuery.fetchNextPage()} style={styles.loadMoreButton}><Text style={styles.loadMoreText}>Xem thêm thành viên</Text></Pressable>
              : profiles.length > 0 ? <Text style={styles.endText}>Bạn đã xem hết kết quả hiện có.</Text> : null}
        </View>
      </View>
    </ScrollView>
  );
...
function ProvinceRow({ province, selected, onPress }: { province: ProvinceOption; selected: boolean; onPress: () => void }) { return <Pressable accessibilityRole="button" onPress={onPress} style={[styles.dropdownOption, selected && styles.dropdownOptionActive]}><Text style={styles.dropdownOptionText}>{province.name}</Text></Pressable>; }
...
const styles = StyleSheet.create({
  ...
  sidebar: { width: luxyLayout.searchSidebarWidth, flexShrink: 0, paddingRight: luxySpacing.xl, borderRightWidth: 1, borderRightColor: luxyColors.border },
  ...
  resetAction: { minWidth: 82, minHeight: 44, borderRadius: luxyRadii.pill, borderWidth: 1, borderColor: luxyColors.ink, alignItems: 'center', justifyContent: 'center', paddingHorizontal: luxySpacing.md },
  resetActionText: { color: luxyColors.text, fontSize: 12, fontWeight: '500' },
  filterSection: { paddingVertical: luxySpacing.md, borderBottomWidth: 1, borderBottomColor: luxyColors.border }, filterSectionLast: { borderBottomWidth: 0 },
  filterHeadingRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, filterHeading: { color: luxyColors.text, fontSize: 13, fontWeight: '600' }, sectionChevron: { color: luxyColors.softMuted, fontSize: 11 }, filterBody: { gap: 5, paddingTop: luxySpacing.sm },
  selectControl: { minHeight: 44, borderWidth: 1, borderColor: luxyColors.borderStrong, borderRadius: luxyRadii.xs, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: luxySpacing.md, backgroundColor: luxyColors.surface },
  selectValue: { color: luxyColors.text, fontSize: 13 }, selectPlaceholder: { color: luxyColors.muted, fontSize: 13 },
  dropdownPanel: { borderWidth: 1, borderColor: luxyColors.border, borderRadius: luxyRadii.xs, backgroundColor: luxyColors.surface }, provinceList: { maxHeight: 260 }, dropdownOption: { minHeight: 40, justifyContent: 'center', paddingHorizontal: luxySpacing.md }, dropdownOptionActive: { backgroundColor: luxyColors.elevatedSubtle }, dropdownOptionText: { color: luxyColors.text, fontSize: 13 },
  locationButton: { minHeight: 44, alignItems: 'flex-start', justifyContent: 'center' }, locationButtonText: { color: luxyColors.text, fontSize: 12, fontWeight: '600' }, helperText: { color: luxyColors.softMuted, fontSize: 11, lineHeight: 16 }, successText: { color: '#166534', fontSize: 11, lineHeight: 16 }, errorText: { color: luxyColors.danger, fontSize: 12, lineHeight: 18 },
  chipRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 }, tagWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 5 },
  filterChip: { minHeight: 32, borderWidth: 1, borderColor: luxyColors.borderStrong, borderRadius: luxyRadii.xs, justifyContent: 'center', paddingHorizontal: 9, backgroundColor: luxyColors.surface }, filterChipActive: { borderColor: luxyColors.ink, backgroundColor: luxyColors.elevatedSubtle }, filterChipText: { color: luxyColors.muted, fontSize: 11 }, filterChipTextActive: { color: luxyColors.text, fontWeight: '600' },
  checkRow: { minHeight: 30, flexDirection: 'row', alignItems: 'center', gap: 7 }, checkbox: { width: 16, height: 16, borderWidth: 1, borderColor: luxyColors.borderStrong, borderRadius: 2, alignItems: 'center', justifyContent: 'center', backgroundColor: luxyColors.surface }, checkboxChecked: { backgroundColor: luxyColors.ink, borderColor: luxyColors.ink }, checkboxDisabled: { backgroundColor: luxyColors.elevatedSubtle }, checkmark: { color: luxyColors.surface, fontSize: 11, lineHeight: 12, fontWeight: '700' }, checkLabel: { flex: 1, color: luxyColors.text, fontSize: 12 }, disabledLabel: { flex: 1, color: luxyColors.softMuted, fontSize: 12 }, comingSoon: { color: luxyColors.softMuted, fontSize: 9 },
