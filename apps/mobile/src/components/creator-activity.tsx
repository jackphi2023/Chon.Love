...
export function CreatorActivityList({ username, compact = false }: ActivityListProps) {
  ...
  const query = useInfiniteQuery({
    ...
    initialPageParam: { beforeAt: null, beforeId: null } satisfies Cursor,
    staleTime: 30_000,
    ...
    getNextPageParam: (page) => {
      ...
      return { beforeAt: last.published_at ?? last.created_at, beforeId: last.post_id } satisfies Cursor;
    },
  });
  ...
  if (query.error) {
    return (
      <ActivityState
        action="Thử lại"
        description={getReadableActivityError(query.error)}
  ...
  if (!posts.length) {
    return <ActivityState description="Creator chưa có bài Hoạt động nào được hiển thị." title="Chưa có Hoạt động" />;
  }
...
export function CreatorActivityCard({ post, compact = false }: { post: CreatorActivityPost; compact?: boolean }) {
  ...
  const dateLabel = formatActivityDate(post.published_at ?? post.created_at);
  ...
  function confirmUnlock() {
    if (!phaseCFeatureFlags.send_gift) {
      setNotice('Mở khóa bằng quà đã sẵn sàng về giao diện và backend, nhưng giao dịch thật đang tắt cho tới khi Google Play Billing được bật.');
      return;
    }
    if (!client || !auth.userId) return;
    Alert.alert(
      `Tặng ${post.required_gift_name_vi ?? 'quà'}?`,
      `Bài này sẽ được mở khóa bằng ${post.required_gift_hearts ?? 0} ❤️. Quà không mua quyền gặp mặt hay liên hệ riêng.`,
      [
        { text: 'Hủy', style: 'cancel' },
        {
          text: 'Xác nhận tặng quà',
          onPress: () => void runAction(async () => {
            await sendGiftAndUnlockCreatorPost(client, post.post_id, crypto.randomUUID());
            await refresh();
  ...
  function confirmReport(target: 'post' | 'image' | 'external_link') {
    ...
    Alert.alert('Báo cáo nội dung', 'Gửi nội dung này tới đội ngũ an toàn MyFan?', [
      { text: 'Hủy', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: () => void runAction(async () => {
          await reportCreatorActivity(client, {
            postId: post.post_id,
            mediaId: target === 'image' ? post.media_id : null,
            target,
            reasonCode: 'other',
          });
  ...
  function confirmOwnerAction(action: 'archive' | 'delete') {
    if (!client) return;
    Alert.alert(
      action === 'archive' ? 'Lưu trữ bài?' : 'Xóa bài?',
      action === 'archive' ? 'Bài sẽ không còn hiển thị công khai.' : 'Bài sẽ bị xóa mềm khỏi MyFan.',
      [
        { text: 'Hủy', style: 'cancel' },
        {
          text: action === 'archive' ? 'Lưu trữ' : 'Xóa',
          style: 'destructive',
          onPress: () => void runAction(async () => {
  ...
  async function openExternalLink() {
    ...
    if (post.external_provider === 'of_tv') {
      Alert.alert('Nội dung bên ngoài MyFan', 'Bạn sắp mở nội dung trên OF.TV. MyFan không gửi email, token, vị trí hoặc dữ liệu cá nhân trong liên kết.', [
        { text: 'Hủy', style: 'cancel' },
  ...
  return (
    <View style={[styles.card, compact && styles.compactCard]}>
      <View style={styles.cardHeader}>
        {avatarQuery.data ? (
          <Image accessibilityLabel={`Ảnh đại diện của ${post.display_name}`} source={{ uri: avatarQuery.data }} style={styles.avatar} />
        ) : (
          <View style={styles.avatarFallback}><Text style={styles.avatarFallbackText}>{post.display_name.slice(0, 1).toUpperCase()}</Text></View>
        )}
        <View style={styles.identity}>
          <View style={styles.nameRow}>
            <Text style={styles.creatorName}>{post.display_name}</Text>
            {post.is_verified ? <Text accessibilityLabel="Creator đã được duyệt" style={styles.verified}>✓</Text> : null}
          </View>
          <Text style={styles.meta}>@{post.username} · {dateLabel}</Text>
        </View>
        <Pressable accessibilityLabel="Báo cáo bài Hoạt động" accessibilityRole="button" onPress={() => confirmReport('post')} style={styles.menuButton}>
      ...
      {post.content_type === 'video' ? (
        <Pressable accessibilityRole="link" onPress={() => void openExternalLink()} style={styles.videoCard}>
          {post.external_provider === 'youtube' && post.external_video_id ? (
            <Image source={{ uri: getYouTubeThumbnail(post.external_video_id) }} style={styles.videoThumbnail} />
          ) : (
            <View style={styles.externalFallback}><Text style={styles.externalProvider}>OF.TV</Text></View>
          )}
          <View style={styles.videoCopy}>
            <Text style={styles.videoProvider}>{post.external_provider === 'youtube' ? 'YouTube' : 'OF.TV · Liên kết ngoài'}</Text>
            <Text numberOfLines={2} style={styles.videoUrl}>{post.external_url}</Text>
            <Text style={styles.openLink}>Mở liên kết ›</Text>
          </View>
        </Pressable>
      ) : null}
        ...
        <View style={styles.mediaFrame}>
          ...
          {isLocked ? (
            <View style={styles.lockedOverlay}>
              ...
              <Text style={styles.lockTitle}>1 ảnh đang được khóa</Text>
              <Text style={styles.lockGift}>
                {post.required_gift_icon_emoji} {post.required_gift_name_vi} · {post.required_gift_hearts} ❤️
              </Text>
              <Pressable accessibilityRole="button" disabled={busy || !post.required_gift_active} onPress={confirmUnlock} style={styles.unlockButton}>
                <Text style={styles.unlockButtonText}>
                  {auth.userId ? `Tặng ${post.required_gift_name_vi ?? 'quà'} để xem ảnh` : `Đăng nhập để tặng ${post.required_gift_name_vi ?? 'quà'}`}
                </Text>
              </Pressable>
            </View>
          ) : null}
      ...
      <View style={styles.footerActions}>
        <Pressable accessibilityRole="button" onPress={() => void Share.share({ message: `${post.body}\n${post.external_url ?? ''}`.trim() })}>
          <Text style={styles.actionText}>Chia sẻ</Text>
        </Pressable>
        {post.content_type === 'video' ? (
          <Pressable accessibilityRole="button" onPress={() => confirmReport('external_link')}><Text style={styles.actionText}>Báo cáo link</Text></Pressable>
        ) : null}
        {post.content_type === 'image' && post.media_id ? (
          <Pressable accessibilityRole="button" onPress={() => confirmReport('image')}><Text style={styles.actionText}>Báo cáo ảnh</Text></Pressable>
        ) : null}
        {post.is_owner ? (
          <>
            <Pressable accessibilityRole="button" onPress={() => confirmOwnerAction('archive')}><Text style={styles.actionText}>Lưu trữ</Text></Pressable>
            <Pressable accessibilityRole="button" onPress={() => confirmOwnerAction('delete')}><Text style={styles.dangerText}>Xóa</Text></Pressable>
          </>
        ) : null}
      </View>
...
function ActivityStatus({ status }: { status: CreatorActivityPost['moderation_status'] }) {
  const labels: Record<CreatorActivityPost['moderation_status'], string> = {
    draft: 'Bản nháp',
    pending_review: 'Đang kiểm duyệt',
    approved: 'Đã đăng',
    rejected: 'Bị từ chối',
    archived: 'Đã lưu trữ',
    deleted: 'Đã xóa',
...
function ActivitySkeleton() {
  return (
    <View style={styles.skeleton}>
      ...
      <Text style={styles.stateDescription}>Đang tải Hoạt động…</Text>
    </View>
  );
}
...
function ActivityState({ title, description, action, onAction }: { title: string; description: string; action?: string; onAction?: () => void }) {
  return (
    <View style={styles.stateCard}>
      ...
      {action && onAction ? <Pressable accessibilityRole="button" onPress={onAction} style={styles.loadMoreButton}><Text style={styles.loadMoreText}>{action}</Text></Pressable> : null}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  lockedOverlay: { position: 'absolute', left: spacing.md, right: spacing.md, bottom: spacing.md, alignItems: 'center', borderRadius: 16, backgroundColor: 'rgba(17,24,39,0.88)', padding: spacing.md, gap: spacing.xs },
  lockIcon: { fontSize: 23 },
  lockTitle: { color: '#FFFFFF', fontSize: 15, fontWeight: '900' },
  lockGift: { color: '#FCE7F3', fontSize: 13, fontWeight: '800' },
  unlockButton: { alignSelf: 'stretch', minHeight: 44, alignItems: 'center', justifyContent: 'center', borderRadius: 999, backgroundColor: colors.primary, marginTop: spacing.sm, paddingHorizontal: spacing.md },
  unlockButtonText: { color: '#FFFFFF', fontSize: 13, fontWeight: '900', textAlign: 'center' },
  ...
  ownerStats: { color: colors.muted, fontSize: 11, paddingHorizontal: spacing.md, paddingBottom: spacing.md },
