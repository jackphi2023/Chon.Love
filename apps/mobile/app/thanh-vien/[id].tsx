...
function usePublicProfileDocumentMetadata(profile: PublicChonProfile | null, avatarUrl: string | null) {
  useEffect(() => {
    ...
    setMeta('meta[name="description"]', 'name', 'description', CHON_PUBLIC_PROFILE_DESCRIPTION);
    setMeta('meta[property="og:title"]', 'property', 'og:title', title);
    setMeta('meta[property="og:description"]', 'property', 'og:description', CHON_PUBLIC_PROFILE_DESCRIPTION);
    setMeta('meta[property="og:type"]', 'property', 'og:type', 'profile');
    setMeta('meta[property="og:url"]', 'property', 'og:url', canonicalUrl);
    setMeta('meta[name="twitter:card"]', 'name', 'twitter:card', 'summary_large_image');
    setMeta('meta[name="twitter:title"]', 'name', 'twitter:title', title);
    setMeta('meta[name="twitter:description"]', 'name', 'twitter:description', CHON_PUBLIC_PROFILE_DESCRIPTION);
    if (avatarUrl) {
      setMeta('meta[property="og:image"]', 'property', 'og:image', avatarUrl);
      setMeta('meta[name="twitter:image"]', 'name', 'twitter:image', avatarUrl);
    }
...
export default function PublicMemberProfilePage() {
  ...
  const routeId = normalizeParam(params.id).trim().toLowerCase();
  ...
  usePublicProfileDocumentMetadata(profile, avatarUrl);
  ...
  if (profileQuery.isLoading) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.muted}>Đang tải hồ sơ thành viên…</Text>
      </View>
    );
  }
  ...
  if (profileQuery.isError) {
    return (
      <View style={styles.centered}>
        <Text accessibilityRole="header" style={styles.errorTitle}>Chưa thể tải hồ sơ</Text>
        <Text style={styles.muted}>Vui lòng thử lại sau.</Text>
        <Pressable accessibilityRole="button" onPress={() => router.replace('/')} style={styles.outlineButton}>
      ...
      <View style={styles.topbar}>
        ...
        <View style={styles.topActions}>
          {!auth.userId ? (
            ...
            <Pressable accessibilityRole="button" onPress={() => router.push('/search')} style={styles.primarySmall}>
              <Text style={styles.primarySmallText}>Kết nối</Text>
            </Pressable>
          )}
        </View>
      </View>
        ...
        <View style={styles.profileCopy}>
          ...
          {profile.looking_for ? (
            <View style={styles.section}>
              ...
              <Text style={styles.body}>{profile.looking_for}</Text>
            </View>
          ) : null}
          ...
          {facts.length > 0 ? (
            <View style={styles.section}>
              ...
              <View style={styles.tags}>{facts.map((fact) => <View key={fact} style={styles.fact}><Text style={styles.factText}>{fact}</Text></View>)}</View>
            </View>
          ) : null}
...
function educationLabel(value: string | null): string | null {
  if (!value || value === 'prefer_not_to_say') return null;
  return ({ high_school: 'THPT', vocational: 'Trung cấp / nghề', college: 'Cao đẳng', bachelors: 'Đại học', masters: 'Thạc sĩ', doctorate: 'Tiến sĩ', other: 'Khác' } as Record<string, string>)[value] ?? value;
}

function relationshipLabel(value: string | null): string | null {
  if (!value || value === 'prefer_not_to_say') return null;
  return ({ single: 'Độc thân', divorced: 'Đã ly hôn', widowed: 'Goá', open: 'Quan hệ mở', complicated: 'Phức tạp' } as Record<string, string>)[value] ?? value;
}
...
const styles = StyleSheet.create({
  ...
  fact: { backgroundColor: '#F8F6F5', borderRadius: luxyRadii.pill, paddingHorizontal: 12, paddingVertical: 8 },
  factText: { color: luxyColors.text, fontSize: 12 },
  ...
  errorTitle: { color: luxyColors.text, fontFamily: luxyTypography.families.display, fontSize: 24 },
});