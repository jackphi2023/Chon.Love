...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const initialProvinceId =
      contextQuery.data?.province_id ?? provincesQuery.data?.[0]?.id ?? null;
  ...
  const profilesQuery = useInfiniteQuery({
    ...
    enabled: Boolean(
      client &&
        auth.userId &&
        contextQuery.data &&
        (mode === 'nearby' || activeProvinceId !== null),
    ),
    staleTime: DISCOVERY_CACHE_MS,
    gcTime: DISCOVERY_CACHE_MS * 2,
    refetchOnMount: false,
        ...
        ListEmptyComponent={
          loadingInitial ? (
            ...
            <Text accessibilityRole="alert" style={styles.error}>
              Không thể tải danh sách Khám phá. Hãy thử lại.
            </Text>
          ) : (
            <Text style={styles.empty}>
              Chưa có hồ sơ phù hợp. Hãy chọn tỉnh khác hoặc quay lại sau.
            </Text>
          )
        }
        ListFooterComponent={
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            ...
            <Text style={styles.description}>
              Người gần nhất được xếp trước. MyFan không hiển thị tọa độ hoặc địa chỉ chính xác.
            </Text>
              ...
              <View style={styles.locationCard}>
                <Text style={styles.locationTitle}>
                  {contextQuery.data?.has_fresh_location
                    ? 'Khoảng cách đang được hiển thị'
                    : 'Khoảng cách đang được ẩn'}
                </Text>
                ...
                <View style={styles.locationActions}>
                  ...
                  >
                    <Text style={styles.primaryButtonText}>
                      {locationBusy ? 'Đang tải…' : 'Cập nhật vị trí'}
                    </Text>
                  </Pressable>
                ...
                {locationMessage ? (
                  <Text accessibilityRole="alert" style={styles.success}>{locationMessage}</Text>
                ) : null}
                {locationError ? (
                ...
                >
                  {(provincesQuery.data ?? []).map((province) => (
                    <Pressable
                      ...
                      style={[
                        styles.provinceChip,
                        selectedProvinceId === province.id && styles.provinceChipActive,
                      ]}
                    >
                      <Text
                        style={[
                          styles.provinceChipText,
                          selectedProvinceId === province.id && styles.provinceChipTextActive,
                        ]}
                      >
                        {province.name}
                      </Text>
            ...
            <View style={styles.resultHeading}>
              <Text style={styles.resultTitle}>
                {mode === 'nearby' ? 'Gần đây' : 'Trong tỉnh/thành'}
              </Text>
...
function DiscoveryCard({ profile }: { profile: DiscoveryProfile }) {
  ...
  const avatarQuery = useQuery({
    queryKey: ['discovery', 'avatar-url', profile.avatar_media_id],
    enabled: Boolean(
      client &&
        profile.avatar_media_id &&
        profile.avatar_storage_bucket &&
  ...
  return (
    <View style={styles.card}>
      {avatarQuery.data ? (
        <Image accessibilityLabel={`Ảnh đại diện của ${name}`} source={{ uri: avatarQuery.data }} style={styles.avatar} />
      ) : (
        <View accessibilityLabel={`Chưa có ảnh đại diện của ${name}`} style={styles.avatarFallback}>
          <Text style={styles.avatarFallbackText}>{name.slice(0, 1).toUpperCase()}</Text>
        </View>
      )}
