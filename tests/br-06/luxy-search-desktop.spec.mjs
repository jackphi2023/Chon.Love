...
test('LX-10/12 desktop Search keeps Seeking rail and activates Favorite/view filters', async ({ browser }, testInfo) => {
  ...
  try {
    ...
    for (const label of [
      'Khu vực', 'Khoảng cách', 'Tùy chọn', 'Lịch sử xem', 'Yêu thích', 'Tuổi', 'Thành viên đang tìm',
      'Cân nặng', 'Tình trạng quan hệ', 'Chiều cao', 'Hút thuốc', 'Uống rượu/bia', 'Học vấn', 'Con cái',
      'Ngôn ngữ', 'Nghề nghiệp', 'Tìm trong hồ sơ',
    ...
    await expect(rail.getByText('Tôi yêu thích', { exact: true })).toBeVisible();
    await expect(rail.getByText('Yêu thích tôi', { exact: true })).toBeVisible();
    ...
    expect(first).not.toBeNull(); expect(second).not.toBeNull(); expect(third).not.toBeNull();
    expect(Math.abs(first.y - second.y)).toBeLessThan(3);
    expect(Math.abs(first.y - third.y)).toBeLessThan(3);
    expect(second.x).toBeGreaterThan(first.x); expect(third.x).toBeGreaterThan(second.x);

    const railBox = await rail.boundingBox();
    const resultsBox = await results.boundingBox();
    expect(railBox).not.toBeNull(); expect(resultsBox).not.toBeNull();
    expect(railBox.x).toBeLessThan(resultsBox.x);
    expect(railBox.width).toBeGreaterThanOrEqual(340); expect(railBox.width).toBeLessThanOrEqual(370);
    await expectNoHorizontalOverflow(page);

    await testInfo.attach('lx12-search-desktop-1280', { body: await page.screenshot({ fullPage: true }), contentType: 'image/png' });
