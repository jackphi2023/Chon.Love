...
async function expectProfileMembershipBadge(page) {
  ...
  expect(Math.abs(badgeBox.height - 32)).toBeLessThanOrEqual(1);
...
test('UI-PRO01 desktop keeps the semantic 32px top-left membership badge, readable facts and stable profile composition', async ({ browser }, testInfo) => {
...
test('UI-PRO01 mobile keeps the semantic 32px top-left membership badge, horizontal album and no overflow at 390px', async ({ browser }, testInfo) => {
